		...
		while ( wpsc_have_products() ) : wpsc_the_product(); ?>
                ...
                <h3 class="prodtitle entry-title"><?php echo wpsc_the_product_title(); ?></h3>
						...
						if (wpsc_have_variation_groups()) {
							// transient for progo_variations_[ID]
							$pid = wpsc_the_product_id();
							$transname = 'progo_pvars_'. $pid;
							if ( false === ( $special_query_results = get_transient( $transname ) ) ) {
								// It wasn't there, so regenerate the data and save the transient
								$oot = '<fieldset>'. _('Product Options', 'wpsc');
								$oot .= '<div class="wpsc_variation_forms"><table>';
								while (wpsc_have_variation_groups()) {
									wpsc_the_variation_group();
									$oot .= '<tr><td class="col1"><label for="'. wpsc_vargrp_form_id() .'">'. wpsc_the_vargrp_name() .'&nbsp;: </label></td>';
									/** the variation HTML and loop */
									$oot .= '<td class="col2"><select class="wpsc_select_variation" name="variation['. wpsc_vargrp_id() .']" id="'. wpsc_vargrp_form_id() .'">';
									while (wpsc_have_variations()) {
										wpsc_the_variation();
										$oot .= '<option value="'. wpsc_the_variation_id() .'" '. wpsc_the_variation_out_of_stock() .'>'. wpsc_the_variation_name() .'</option>';
									}
									$oot .= '</select></td></tr>';
								}
								$oot .= '</table></div></fieldset>';
														
								set_transient( $transname, $oot, 60*60*12 );
							}
							// Use the data like you would have normally...
							$oot = get_transient( $transname );
							echo $oot;
						} ?>
					...
					<form onsubmit="submitform(this);return false;" action="<?php echo wpsc_this_page_url(); ?>" method="post" name="product_<?php echo wpsc_the_product_id(); ?>" id="product_extra_<?php echo wpsc_the_product_id(); ?>">
						...
						<div class="product_description">
							<?php echo str_replace("~", "", wpsc_the_product_description()); ?>
						</div><!--close product_description -->	
