      ...
      <tr class="product_row product_row_<?php echo wpsc_the_cart_item_key(); ?> <?php echo $alt_class;?>">
         ...
         <td class="wpsc_product_remove wpsc_product_remove_<?php echo wpsc_the_cart_item_key(); ?>">
            <form action="<?php echo get_option('shopping_cart_url'); ?>" method="post" class="adjustform remove">
               ...
               <input type="hidden" name="key" value="<?php echo wpsc_the_cart_item_key(); ?>" />
               <input type="hidden" name="wpsc_update_quantity" value="true" />
               <input type="submit" value="<?php _e('X REMOVE', 'wpsc'); ?>" name="submit" class="nobtn" />
            </form>
         </td>
      </tr>
   <?php endwhile; ?>
        ...
        <?php if(wpsc_checkout_form_is_header() == true){
			...
			<tr>
               ...
               <td>
				  ...
				  if( $wpsc_checkout->checkout_item->type == 'dateofbirth' ) {
					?><select name='collected_data[<?php echo $wpsc_checkout->checkout_item->id ?>][month]'>
                    <option value='-1'>Month</option>
                    <?php
					$savedd = '';
					if ( isset($_SESSION['wpsc_checkout_saved_values'][$wpsc_checkout->checkout_item->id]['month']) ) {
						$savedd = $_SESSION['wpsc_checkout_saved_values'][$wpsc_checkout->checkout_item->id]['month'];
					}
					$months = array(
						1 => 'January',
						2 => 'February',
					...
					foreach ( $months as $dm => $dn ) {
						$sel = ( $dm == $savedd ? " selected='selected'" : "" );
						echo "<option value='$dm'$sel>$dn</option>";
					}
					?>
                    </select>&nbsp;&nbsp;<select name='collected_data[<?php echo $wpsc_checkout->checkout_item->id ?>][day]'>
                    <option value='-1'>Day</option>
                    <?php
					$savedd = '';
					if ( isset($_SESSION['wpsc_checkout_saved_values'][$wpsc_checkout->checkout_item->id]['day']) ) {
						$savedd = $_SESSION['wpsc_checkout_saved_values'][$wpsc_checkout->checkout_item->id]['day'];
					}
					for($dd = 1; $dd < 32; $dd++) {
						$sel = ( $dd == $savedd ? " selected='selected'" : "" );
						echo "<option value='$dd'$sel>$dd</option>";
					}
					?>
                    </select>&nbsp;&nbsp;<select name='collected_data[<?php echo $wpsc_checkout->checkout_item->id ?>][year]'>
                    <option value='-1'>Year</option>
                    <?php
					$thisyear = intval(date('Y'));
					$savedd = '';
					if ( isset($_SESSION['wpsc_checkout_saved_values'][$wpsc_checkout->checkout_item->id]['year']) ) {
						$savedd = $_SESSION['wpsc_checkout_saved_values'][$wpsc_checkout->checkout_item->id]['year'];
					}
					for($dd = $thisyear; $dd > ($thisyear-101); $dd--) {
						$sel = ( $dd == $savedd ? " selected='selected'" : "" );
						echo "<option value='$dd'$sel>$dd</option>";
					}
					?>
...
?>
      ...
      <?php if(wpsc_has_tnc()) : ?>
         <tr>
            <td colspan='2'>
                <label for="agree"><input id="agree" type='checkbox' value='yes' name='agree' /> <?php printf(__("I agree to the <a target='_blank' href='%s' class='termsandconds'>Terms and Conditions</a>", "wpsc"), get_permalink(10)); ?></label>
               </td>
         </tr>
      <?php endif; ?>
      </table>
